...
1. **テストの命名規則**
   - テストメソッド名は以下のような3要素を、アンダーバー `_` で結合した命名にする：
     1. テスト対象のメソッド名。元のメソッド名をそのまま記載する
     2. 正常系か、異常系か。メソッド中に例外が発生するケースがある場合、例外が発生しない場合には「正常系」、例外が発生する場合には「異常系」と記載する。例外発生しないメソッドの場合にはこの項目は省略する
     3. 期待する動作や結果について。dataProviderを指定していない場合、「処理の概要と返却結果」を簡単に書く。dataProviderを指定している場合、「⚪︎⚪︎の検証」という書き方をする
   - 例：
     - DTOクラスの場合、 `getCognitiveUnitValue_メンバ変数cognitiveUnitValueを返す`
     - シミュレータの場合、 `run_シミュレーションを実行してフォーマットされた結果の配列を返す`
     - dataProviderを使っている場合、 `getDisplayText_入力値と変換される表示テキストの検証`
     - 例外発生する場合、 `createFraction_異常系_分母が0の時に例外をスローする`
     - 例外が発生する事もあるが正常系を検証する場合、 `createFraction_正常系_Fractionオブジェクトを生成して返す`
   - テストメソッドの先頭がtestでないため、phpunitにユニットテストである事を認識させるためにphpdocには@testアノテーションを必ず付与する
